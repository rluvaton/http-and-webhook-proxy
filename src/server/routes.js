    ...
    return new Promise((resolve, reject) => {
        ...
        fastify.io
            ...
            .emit(`http-${request.id}`, {
                ...
                warnInCaseResponseDataIsBufferAndNoContentTypeHeader(response, request.log);
                ...
                try {
                    ...
                    request.log.error({error: e}, 'Some info about the current request')
                    reject(e);
                }
...
function warnInCaseResponseDataIsBufferAndNoContentTypeHeader(response, logger) {
    if (!response) {
        return;
    }
    ...
    logger.warn({response}, `Content-Type header is missing and the response data is a buffer, setting Content-Type to be 'application/octet-stream'`)
}
        ...
        fastify.io.to(urlPrefix).fetchSockets()
            .then((sockets) => {
                sockets.map(s => s.on('ws-message', (data) => {
                    ...
                    req.log.info('new server to client data');
                    connection.socket.send(serverToClientData);
                }));
            })

        connection.socket.on('message', message => {
            req.log.info('new client to server data');
